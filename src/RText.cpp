...
struct GlyphDef
{
    ...
    static constexpr int Triangles = N;
    char c;
    metric m;
    uint8_t t[ Indicies * 2 ]; // each index has an X and y component
};

// these glyphs were built using gewellt
static GlyphDef<8> glyphArr[] = {
...
,{ '-',    18,5,10,20,38,-9,23,51,    12,5,6,3,16,1,9,0,0,2,8,5,9,0,13,1,10,3,0,1,0,0,8,0,18,5,18,5,18,5,17,1,7,0,18,0,17,11,17,11,8,12,16,0,18,4,12,5,}
,{ '.',    7,9,15,9,38,-4,21,51,    0,5,9,0,2,0,7,9,7,9,7,9,12,10,10,12,11,12,4,0,4,1,4,0,8,1,5,11,0,3,6,7,6,7,2,8,18,9,21,5,21,8,7,8,7,3,7,3,}
...
,{ 'E',    30,42,5,42,38,-14,4,51,    30,0,20,7,0,0,8,39,0,42,1,10,0,6,11,20,0,26,0,1,12,2,3,9,30,22,15,15,5,22,21,35,30,39,25,42,4,0,0,6,5,15,0,41,16,35,30,41,}
...
,{ 'H',    28,42,5,42,38,-14,4,51,    0,25,4,20,6,37,0,11,7,17,3,0,0,13,0,26,21,20,0,23,7,40,0,42,22,42,22,22,28,27,25,0,28,9,19,5,27,42,22,40,27,30,27,0,27,26,15,22,}
...
,{ 'K',    34,42,5,42,38,-14,4,51,    10,19,0,25,7,28,22,30,24,40,33,42,7,23,0,12,0,30,26,33,20,36,19,26,7,29,0,24,0,42,0,42,4,33,7,40,18,31,6,19,26,0,5,22,0,8,5,1,}
...
,{ 'M',    30,42,4,42,38,-15,4,51,    5,0,0,10,15,16,6,13,21,17,15,29,24,0,29,5,16,17,6,16,0,7,0,39,3,20,5,41,0,41,29,0,29,36,22,20,16,17,21,17,21,13,26,23,30,41,24,41,}
,{ 'N',    28,42,5,42,38,-14,4,51,    0,41,5,41,0,32,24,27,22,0,28,1,9,18,17,24,7,4,0,34,0,21,7,18,27,10,27,42,21,27,22,42,8,12,28,41,0,36,5,23,4,38,0,28,0,0,11,1,}
...
,{ 'T',    34,42,2,42,38,-17,4,51,    19,5,19,18,12,14,9,3,0,0,0,5,13,15,20,17,18,37,14,42,21,40,14,26,20,36,14,21,14,31,15,9,20,0,34,0,18,11,20,0,1,0,32,0,26,4,33,4,}
...
,{ 'c',    30,36,4,35,38,-15,7,51,    28,23,27,31,16,36,9,2,0,12,5,4,24,11,18,0,30,7,15,0,16,3,8,4,15,36,27,28,10,34,0,25,7,27,5,9,1,11,12,0,0,22,19,36,0,23,5,32,}
...
,{ 'f',    30,46,4,46,38,-15,2,51,    8,45,13,45,8,34,25,9,19,15,29,16,7,33,13,45,14,21,14,22,8,31,8,17,0,12,13,21,22,12,16,0,7,7,30,2,2,17,0,13,3,14,15,15,8,12,11,1,}
...
,{ 'k',    29,46,7,46,38,-12,2,51,    18,14,12,27,29,10,15,19,8,27,14,28,17,29,8,27,19,40,0,0,4,14,6,0,18,34,20,45,27,46,6,46,0,46,3,39,0,0,0,45,9,29,5,37,3,39,5,43,}
...
void RText::Bind(short platWidth_, short platHeight_)
{
    platWidth = platWidth_;
    platHeight = platHeight_;

    assert(!bound);
    if(bound) return;

    const short boxWidthPX = platWidth * 4 / 5;
    const float glySizePX = float(boxWidthPX) / float(cols);
    const float scaleFact = glySizePX / glyphArr[0].m.horiAdvance;

    glm::mat3x3 contextMX =
        glm::translate(
            glm::scale(
                glm::mat3x3(1),
                glm::vec2( scaleFact, scaleFact ) // scale glyphs to fit cols in width
            ),
            glm::vec2(0, glyphArr[0].m.vertAdvance) // move origin from glyph baseline to glyph top
        );

    std::vector< glm::tvec2<GLshort> > posVerts;
    posVerts.resize( textLength * GlyphDef<8>::Indicies );

    int iWord = 0;
    glm::vec2 cursor(0,0);
    glm::mat3x3 glyphMX;

    // scan and measure text, inserting newlines
    for(int iChar=0; iChar<textLength; iChar++)
    {
        if(text[iChar] == '\n') // force newline
        {
            cursor = glm::vec2( 0, cursor.y + glyphArr[0].m.vertAdvance );
            continue;
        }

        // skip spaces on left edge
        if(text[iChar] == ' ' && cursor.x < .01) continue;

        // to perform line-wrapping, recognize word-delimiters as either space or slash
        if(iChar > 0)
        {
            if(text[iChar] != ' ' && text[iChar -1] ==  ' ') iWord = iChar;
            if(text[iChar] != '/' && text[iChar -1] ==  '/') iWord = iChar;
        }

        // lookup glyph, but skip if not found
        auto pGlyph = (GlyphDef<8>*) bsearch(
            &text[iChar],
            glyphArr,
            sizeof(glyphArr) / sizeof(GlyphDef<8>),
            sizeof(GlyphDef<8>),
            comparator
        );
        if(!pGlyph) continue;

        // create origin for new glyph, and newline if past width
        auto glyphOrigin = glm::vec2( pGlyph->m.horiBearingX, -pGlyph->m.horiBearingY ); // top-down glyph
        glyphMX = glm::translate( contextMX, glyphOrigin + cursor );

        // if there is a word-break on the last line and we've gone past the line width, relocate cursor adn restart word
        auto glyphEdgePX = glyphMX[2].x + scaleFact * pGlyph->m.width;
        if(iChar - iWord < cols && glyphEdgePX > boxWidthPX)
        {
            cursor = glm::vec2( 0, cursor.y + glyphArr[0].m.vertAdvance * 6 / 5 ); // newline, with spacing
            iChar = iWord -1; // restart word
            continue;
        }

        // transform points and copy into vertex-buffer
        struct vert_t { uint8_t x, y; };
        vert_t* pv = (vert_t*)&pGlyph->t;
        for(int j=0;j<GlyphDef<8>::Indicies;j++)
        {
            auto r = glyphMX * glm::tvec3<float>( pv[j].x, pv[j].y, +1 ); // +1 for normalization
            posVerts[ iChar * GlyphDef<8>::Indicies + j ] = { r.x, r.y };
        }

        cursor.x += pGlyph->m.horiAdvance;
    }

    short heightPX = glyphMX[2].y + scaleFact * glyphArr[0].m.vertAdvance;

    position = glm::vec3( platWidth / 10, platHeight / 2 - heightPX / 2, 0 ); // center

    const short borderPX = 3;
    const GLshort qqq[12] =
        {
            GLshort(-borderPX),GLshort(-borderPX),
            GLshort(-borderPX),GLshort(heightPX+borderPX),
            GLshort(boxWidthPX+borderPX),GLshort(-borderPX),
            GLshort(boxWidthPX+borderPX),GLshort(-borderPX),
            GLshort(-borderPX),GLshort(heightPX+borderPX),
            GLshort(boxWidthPX+borderPX),GLshort(heightPX+borderPX)
        };
    memcpy( quad, qqq, sizeof(qqq) );

    // bind to vbo
    gl9GenBuffers( 1, &boVerts );
    gl9BindBuffer( GL_ARRAY_BUFFER, boVerts );
    gl9BufferData( GL_ARRAY_BUFFER, posVerts.size() * sizeof(glm::tvec2<GLshort>), posVerts.data(), GL_STATIC_DRAW );
...
void RText::Render()
{
    glm::mat4 foo = glm::translate( position );
    gl9MultMatrixf(glm::value_ptr(foo));

    {
        const glm::vec4 grey = {.25f,.25f,.25f,.75f};
        gl9Color4fv( glm::value_ptr( grey ));

        gl9BlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
        gl9Enable( GL_BLEND );

        gl9ClientStateDisabler objectVertArrState( GL_VERTEX_ARRAY );
        gl9VertexPointer( 2, GL_SHORT, 0, quad );
        gl9DrawArrays( GL_TRIANGLES, 0, 6 );
...

    {
        ...
        gl9Color3fv( glm::value_ptr( white ));

        gl9ClientStateDisabler objectVertArrState( GL_VERTEX_ARRAY );
        gl9BufferUnbinder objectVerts( GL_ARRAY_BUFFER, boVerts );
        gl9VertexPointer( 2, GL_SHORT, 0, nullptr );
        gl9DrawArrays( GL_TRIANGLES, 0, textLength * GlyphDef<8>::Indicies );
    }
}
